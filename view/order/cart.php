                                ...
                                <div class="row align-items-center">
                                    <div class="col-6 col-md-6 col-lg-7">
                                        <div class="d-flex">
                                            ...
                                            <div class="ms-3">
                                                <!-- title -->
                                                <a href="index.php?act=product_details&id=<?= $id ?>" class="text-inherit">
                                    ...
                                    <div class="col-4 col-md-3 col-lg-3">
                                        ...
                                        <div class="input-group input-spinner">
                                            <input type="button" value="-" onclick="btnMinus(this, <?= $id ?>)" class="button-minus btn btn-sm" data-field="quantity" />
                                            <input type="number" min="1" max="<?= $quantity ?>" value="<?= $quantityOrder ?>" name="quantity" class="quantity-field form-control-sm form-input" />
                                            <input type="button" value="+" onclick="btnPlus(this, <?= $id ?>)" class="button-plus btn btn-sm" data-field="quantity" />
                                        </div>
                                    </div>
                                    <!-- price -->
                                    <div class="col-2 text-lg-end text-start text-md-end col-md-2">
                                        <span class="fw-bold"><?= number_format(intval($price) * intval($quantityOrder)) ?> VND</span>
                                    </div>
                                </div>
                            </li>
                            <?php 
                                    $total += (intval($price) * intval($quantityOrder));
                                }
                            ?>
                        </ul>
                        <!-- btn -->
                            ...
                            <div class="card mb-2">
                                ...
                                <ul class="list-group list-group-flush">
                                    ...
                                    <li class="list-group-item d-flex justify-content-between align-items-start">
                                        ...
                                        <span class="fw-bold text-danger"><?= number_format($total) ?> VND</span>
                                    </li>
                                </ul>
                            </div>
...
<script>
    function updateQuantity(idProduct, quantity) {
        $.ajax({
            type: 'POST',
            url: './view/order/update_quantity.php',
            data: {
                id: idProduct,
                quantity: quantity
            }, 
            success: function(response) {
                $.post('./view/order/table_cart.php', function(data){
                    $('#order').html(data);
                })
            },
    ...
    function removeFromCart(idProduct) {
        if(confirm("Bạn có đồng ý xóa không ?")) {
            $.ajax({
                type: 'POST',
                url: './view/order/remove_from_cart.php',
                data: {
                    id: idProduct
                }, 
                success: function(response) {
